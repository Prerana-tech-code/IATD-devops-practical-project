...
let inventoryItems = [
  ...
  { name: "Pants", category: "Clothing", amount: 103, restocked: "19/09/2024" },
  {
    ...
    amount: 4,
    restocked: "12/03/2023",
  },
  {
    name: "Cat. 5 Network Cable 20m",
    category: "Electronics",
    amount: 13,
    restocked: "10/10/2024",
  },
];
    ...
    case 1: {
      ...
      switch (choice) {
        case 0: {
          ...
          let itemIndex = -1;
          do {
            itemName = readlineSync.question(
              wrapString("Enter the name of the item to add/remove stock for: ")
            );
            for (let i = 0; i < inventoryItems.length; i++) {
              if (
                inventoryItems[i].name.toLowerCase() === itemName.toLowerCase()
              ) {
                itemIndex = i;
                break;
              }
            ...
            if (itemIndex < 0) {
              logWrapped(
                `ERROR: Item of name ${itemName} not found. Please enter the name of an item already tracked by this system.`
              );
            }
          } while (itemIndex < 0);

          logWrapped(
            `There are currently ${inventoryItems[itemIndex].amount} ${inventoryItems[itemIndex].name} in stock. Last stock change was on ${inventoryItems[itemIndex].restocked}`
          );
          ...
          switch (action) {
            case 0: {
              const amount = readlineSync.questionInt(
                wrapString(
                  `Enter the amount of ${inventoryItems[itemIndex].name} to add: `
                )
              );
              ...
              readlineSync.keyInPause(
                wrapString(
                  `${inventoryItems[itemIndex].name} successfully updated, press q to return to main menu...`
                ),
                { limit: ["q"], guide: false }
              );
              break;
            ...
            case 1: {
              ...
              do {
                amount = readlineSync.questionInt(
                  wrapString(
                    `Enter the amount of ${inventoryItems[itemIndex].name} to remove: `
                  )
                );
                if (amount > inventoryItems[itemIndex].amount)
                  logWrapped(
                    `ERROR: There are only ${inventoryItems[itemIndex].amount} ${inventoryItems[itemIndex].name}, you cannot remove more than this.`
                  );
        ...
        case 1: {
          ...
          const itemName = readlineSync.question(
            wrapString("Enter the name of the new item to add: ")
          );

          let category = 0;
          let cachedLength = 0;
          do {
            category = readlineSync.keyInSelect(
              ...
              do {
                isValid = addCategory(
                  readlineSync.question(
                    wrapString("Enter the name of the category to add: ")
                  )
                );
              } while (!isValid);
            }
          ...
          do {
            amount = readlineSync.questionInt(
              wrapString("Enter the amount of stock to add: ")
            );
            if (amount < 1)
              logWrapped("ERROR: you must add at least one of this item.");
          } while (amount < 1);
          ...
          logWrapped(
            `Successfully added item ${itemName} with the following details:`
          );
...
function enterStockChangeDate() {
  ...
  do {
    date = readlineSync.question(
      wrapString(
        "Enter the date of this stock change using the format DD/MM/YYYY: "
      )
    );
